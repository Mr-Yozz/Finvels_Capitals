...
@section('content')
<h2 class="text-primary mb-3">Add Member</h2>
...
<form action="{{ route('members.store') }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <input type="text" name="aadhaar_encrypted" class="form-control" value="{{ old('aadhaar_encrypted') }}" required>
        @error('aadhaar_encrypted') <small class="text-danger">{{ $message }}</small> @enderror
    </div>

    <div class="mb-3">
        <label class="form-label text-primary">PAN</label>
        <input type="text" name="pan_encrypted" class="form-control" value="{{ old('pan_encrypted') }}" required>
